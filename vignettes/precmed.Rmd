...
The recommended workflow of analyses to identify treatment effect heterogeneity with several candidate scoring methods follows the steps below:

1.	Compare up to 5 methods (e.g., Poisson regression, boosting) for estimating the CATE via cross-validation (function `catecv()`).
2.	Select the best method using 3 metrics:
  * Compare the steepness of the validation curves in the validation samples across methods (function plot()). Two side-by-side plots are generated, visualizing the estimated average treatment effects in a series of nested subgroups. On the left side the curve is shown for the training set, and on the right side the curve is shown for the validation set. Each line in the plots represents one scoring method (e.g., boosting, randomForest) specified under score.method.
  * The area between curves (ABC)  (function abc()) .quantifies a model’s ability to capture treatment effect heterogeneity. Higher ABC values are preferable as they indicate that more treatment effect heterogeneity is captured by the scoring method.
  * Compare the distribution of the ATE in mutually exclusive subgroups in the validation samples (function boxplot()). Provides box plots which depict distributions of estimated ATEs for each multi-category subgroup in the validation set across all cross-validation iterations. The subgroups are mutually exclusive and are categorized by the CATE score percentiles prop.multi specified in catecv().
