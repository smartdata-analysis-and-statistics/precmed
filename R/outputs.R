...
#   Date			By			Description
# --------		--------	-----------------------------
#  19OCT2020  pj      Start the script with cvARR.function()
#  20OCT2020  pj      Finish cleaning cvARR.function()
#  20OCT2020  gs      Add pmARR.function()
#  23OCT2020  gs      Change ARR to count
#  17NOV2020  pj      Add arguments "minPS", "maxPS" to both outputs functions and the corresponding inner functions
#                     Add max.iter to cvcount.function() inherited from balance.split()
#  03DEC2020  pj      Inherit glm.ps and twoarm parameters in intx() and cv()
#  04JAN2021  gs      Add warning in cv() and pm() to increase n.trees if best.iter=n.trees in intx()
#  18JAN2021  gs      Add arguments check at the beginning of pm() and cv()
#                     Distinguish arguments max.iter and max.iterNR
#  20JAN2021  pj      Add and modify more arugment checks at the beginning of pm() and cv()
#  21JAN2021  gs      Remove ps.method argument from balance.split()
#  02FEB2021  gs      Update argument names
#  02FEB2021  pj      Move drcount missing logRR warning to pm and cv
#                     Refine cvcount.plot()
#  03FEB2021  gs      Add warning in cv if convergence=F
#  22FEB2021  gs      Add commented changes to cate.model ps.model in cvcount()
#  23FEB2021  pj      Add score.method argument to cv(), pm(); rename the trt1-4 output to ate (more intuitive)
#  24FEB2021  gs      Add changes x.cate, x.ps and update argument descriptions
#                     Change warning for recoding trt 0/1
#  04MAR2021  gs      Wrap arguments check beginning of pmcount() and cvcount() into a function
#  09MAR2021  gs      Update argument descriptions
#  10MAR2021  pj      Modify plot.cvcount() into lineplot.cvcount() and add boxplot.cvcount()
#  31MAR2021  pj      Add poisson/GAM as initial.predictor.method in addition to GBM
#  12APR2021  gs      Add coefficients output to pmcount()
...
#                     Add a higher.y argument
#  27APR2021  gs      Add abc calculation in cvcount and lineplot
#  04MAY2021  gs      abc calculation based on fixed prop.abc with 100 values
#  07MAY2021  gs      No abc results with abc = F, improve documentation cvcount()
#  10MAY2021  gs      Change ps.method options, update cvcount + pmcount argument and value descriptions
#                     Have only one set.seed call for cvcount and pmcount
#  12MAY2021  pj      Update lineplot, boxplot, abc arguments and value descriptions
#  17MAY2021  gs      Simplify fit output from pmcount(), update description of functions
#                     Save output of cvcount as "PrecMed" object and define boxplot and plot for PrecMed object
#  19MAY2021  pj      Revise all documentation
#                     Change "test" to "valid" in code or "validation" in text
#                     Add examples, seealso, and references in the documentation of each function
#  24MAY2021  pj      Format the documentation and code to a shorter width for easier reading
#  03JUN2021  pj      Change cvfit to x as an argument and specify .data$ to variables in plot, boxplot, abc
#  14JUN2021  mp      Chang abc calculation based on log ate
#  30JUN2021  gs      Change linetype for negBin in grayscale plot, add names to pm$ate.METHOD
#  30JUN2021  pj      Improve ABC text in plot and x-label in boxplot
#  07JUL2021  pj      Minor fixes
#  08JUL2021  gs      Added drcount.inference as a separate function
#  13JUL2021  gs      More efficient column naming in cvcount based on code by Dasom
#  14JUL2021  gs      Fix bug in plot with show.abc == FALSE and x$abc == FALSE
#  14JUL2021  pj      Minor changes to drcount.inference()
#  19JUL2021  gs      drcount don't have rate.ratio as output anymore
#  21JUL2021  pj      Remove tibble and set plot.boot = TRUE in drcount.inference()
#                     Move abc results to the legend in plot() instead of in bubbles
#                     Remove magrittr and matrixStats from importing packages
#  26AUG2021  gs      Add initial.predictor.method in arg.check
#  27AUG2021  gs      Minor bug fix in drcount.inference
#  09SEP2021  gs      Minor edits to gbm trees warning message (line break fix)
#                     Remove ate.est.train/valid.cv output from cvsurv() and update plot() accordingly
#  14SEP2021  gs      Update R documentation following change above
#  20SEP2021  gs      Fix legend bug in plot.PrecMed()
#  06OCT2021  pj      Set seed once in cvcount, pmcount, drcount.inference and remove other set.seed functions
...
#  01DEC2021  gs      Clarify label of subgroups in boxplot
#  01DEC2021  pj      Update R documentation
#  03DEC2021  gs      Update prop.multi documentation
#                     Clarify subgroup and x-axis in boxplot
#  09DEC2021  pj      Update plot documentation to align with plot for cvsurv
#  13DEC2021  gs      Fix axes in boxplot, remove legend
#                     Always extract overall ATE by cv iterations in cvcount() - to be used as overall ATE in boxplot
#                     Organize output code of drcount.inference like in drsurv.
#  15DEC2021  pj      Add valid.only, legend.position, xlim in plot
#  17DEC2021  gs      Always plot overall ATE in lineplot
#  07JAN2022  pj      Revise plot.PrecMed() to match with plot.surv.PrecMed()
#  01FEB2022  gs      Output cv.n from cvcount; update error message in plot when cv.i > cv.n
#  04FEB2022  pj      Fix \link in documentation and add .data$
#  03MAR2022  pj      Remove plot.PrecMed()
...
#  11MAR2022  gs      Update cvcount, pmcount, drcount.inf arguments & documentation to match wrapper
#  24MAR2022  pj      Create wrapper for arg.checks() to distinguish common and specific args based on outcome type
#  22APR2022  pj      Minor typo fix in plot
#  25MAY2022  gs      Minor changes in dr.inference (output warning)
#  25MAY2022  pj      Revise verbose argument in cv() and dr.inference() to integer values
#  26MAY2022  pj      Change abc() output from combined vector to uncombined matrix
#  01JUN2022  pj      Fix dr.inference() verbose tp control only text outputs
#  06JUL2022  gs      drcount rate language fix + drcount.inference output description
#  11JUL2022  gs      Added fgam output to cvcount()
#  13JUL2022  gs      Check pm() arguments
#  22JUL2022  gs      Update drinf function description
#  15AUG2022  gs      Transfer abc() to output_surv.R
# ------------------------------------------------------------------
